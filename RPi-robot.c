...
int main(int argc, char **argv)
{
    ...
    volatile double rpm_r = 0.0;
    volatile double rpm_l = 0.0;
    volatile double batt_voltage = 0.0;
    volatile double current_consumption= 0.0;
    ...
    struct query_arg_struct args_query = { .rpm_r = &rpm_r, .rpm_l = &rpm_l
        , .running = &running
    ...
    while (running)
    {
        ...
        if (mode == 0)
        {
            int res = keypress_mode_stepwise(c, &running, &left_motor, &right_motor);
            if (res == 1)
            {
                mode = 1;
                timeout(100);
